...
def plot_angle_axes(t, thetas, omega_axes_1, omega_axes_2, omega_axes_3):
    ...
    axs[0].plot(t, omega_axes_1)
    axs[0].set_ylabel("ω1")

    axs[1].plot(t, omega_axes_2)
    axs[1].set_ylabel("ω2")

    axs[2].plot(t, omega_axes_3)
    axs[2].set_ylabel("ω3")
    ...
    axs[0].set_title("Orientation vs Time")
...
def plot_body_angular_velocity(t, omega_body):
    ...
    omega_1 = omega_body[0,:]
    omega_2 = omega_body[1,:]
    omega_3 = omega_body[2,:]

    fig, axs = plt.subplots(3,1, figsize=(8,8))

    axs[0].plot(t, omega_1)
    axs[0].set_ylabel(r'$\omega_{b1}$ (rad/s)')

    axs[1].plot(t, omega_2)
    axs[1].set_ylabel(r'$\omega_{b2}$ (rad/s)')

    axs[2].plot(t, omega_3)
    axs[2].set_ylabel(r'$\omega_{b3}$ (rad/s)')
    axs[2].set_xlabel("Time (s)")
...
def plot_error_metric(t, error):
    ...
    plt.xlabel("Time (s)")
    plt.ylabel("d^2")
    plt.title("Error Metric vs Time")
...
def calculate_angle_axes(Rs):
    ...
    for i in range(N):
        ...
        omega, theta = AngleAxis3(expc3)

        thetas[i] = theta
        omega_axes_1[i] = omega[0]
        omega_axes_2[i] = omega[1]
        omega_axes_3[i] = omega[2]
...
def calculate_body_angular_velocity(t, Rs):
    ...
    for i in range(N-1):
        ...
        expc3_hat = MatrixLog3(R_rel)
        expc3 = so3ToVec(expc3_hat)

        dt = t[i+1] - t[i]

        omega_body[:,i] = expc3 / dt

    # last value same as previous
